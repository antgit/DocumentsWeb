...
{
    /// <summary>
    /// Контроллер объектов "Поставщики"
    /// </summary>
    [MultiAuthorize(Roles = new[] { Uid.GROUP_GROUPWEBUSER })]
    public class SupplyerController : CoreController<Agent>
    {
        public SupplyerController()
        {
            Name = WebModuleNames.WEB_SUPPLYER;
            RootHierachy = Hierarchy.SYSTEM_AGENT_SUPPLIERS;
        }
        ...
        public ActionResult Edit([ModelBinder(typeof(DevExpressEditorsBinder))] ClientModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    //New
                    Hierarchy hierarchy = WADataProvider.WA.Cashe.GetCasheData<Hierarchy>().ItemCode<Hierarchy>(Hierarchy.SYSTEM_AGENT_SUPPLIERS);
                    hierarchy.ContentAdd(obj, true);
        ...
        {
            ClientModel.ToTrash(id, Hierarchy.SYSTEM_AGENT_SUPPLIERS);
        }
        ...
        public void ChangeState(int id, int state)
        {
            switch (state)
            {
                case State.STATEACTIVE:
                    ClientModel.SetStatetDone(id, Hierarchy.SYSTEM_AGENT_SUPPLIERS);
                    break;
                case State.STATENOTDONE:
                    ClientModel.SetStateNotDone(id, Hierarchy.SYSTEM_AGENT_SUPPLIERS);
                    break;
                case State.STATEDENY:
                    ClientModel.SetStateDeny(id, Hierarchy.SYSTEM_AGENT_SUPPLIERS);
                    break;
            }
        }
        ...
        {
            ClientModel.CreateCopy(id, Hierarchy.SYSTEM_AGENT_SUPPLIERS);
        }
        ...
        {
            GridViewSettings settings = new GridViewSettings { Name = "Поставщики" };
            ...
            IEnumerable coll = ClientModel.GetSuppliers();
