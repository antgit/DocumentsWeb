...
{
    /// <summary>
    /// Контроллер объектов "Банки"
    /// </summary>
    [MultiAuthorize(Roles = new[] { Uid.GROUP_GROUPWEBUSER })]
    public class BankController : CoreController<Agent>
    {
        public BankController()
        {
            Name = WebModuleNames.WEB_BANKS;
            RootHierachy = Hierarchy.SYSTEM_AGENT_BANKS;
        }
        [HttpGet]
        ...
        public ActionResult IndexPartial(bool refresh = false)
        {
            ...
            if (roots.Contains(0))
            {
                return PartialView(ClientModel.GetCollectionBanks(RootHierachy, false));
                //list = ClientModel.GetCollection(HierarchyModel.GetLinkedHierarchies(RootHierachy).Select(s => s.Code).ToArray<string>());
            }
            list = ClientModel.GetCollectionBankWONested(roots);
        ...
        {
            ClientModel model = new ClientModel { Id = 0, KindId = Agent.KINDID_BANK };
        ...
        {
            ClientModel model = id == 0 ? new ClientModel { Id = 0, KindId = Agent.KINDID_BANK } : ClientModel.GetObject(id);
            WADataProvider.ModelsCache.Add(model.ModelId, model);
        ...
        public ActionResult ControlView([ModelBinder(typeof(DevExpressEditorsBinder))] ClientModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    //New
                    Hierarchy hierarchy = WADataProvider.WA.Cashe.GetCasheData<Hierarchy>().ItemCode<Hierarchy>(Hierarchy.SYSTEM_AGENT_BANKS);
        ...
        {
            ClientModel.ToTrash(id, Hierarchy.SYSTEM_AGENT_BANKS);
        }
        ...
        public void ChangeState(int id, int state)
        {
            switch (state)
            {
                case State.STATEACTIVE:
                    ClientModel.SetStatetDone(id, Hierarchy.SYSTEM_AGENT_BANKS);
                    break;
                case State.STATENOTDONE:
                    ClientModel.SetStateNotDone(id, Hierarchy.SYSTEM_AGENT_BANKS);
                    break;
                case State.STATEDENY:
                    ClientModel.SetStateDeny(id, Hierarchy.SYSTEM_AGENT_BANKS);
                    break;
            }
        }
        ...
        {
            ClientModel.CreateCopy(id, Hierarchy.SYSTEM_AGENT_BANKS);
        }
        ...
        public ActionResult ExportTo(string type, string subtype)
        {
            GridViewSettings settings = new GridViewSettings
                {
                    Name = "dxGridViewAgents",
                    KeyFieldName = "Id",
                    CallbackRouteValues = new {Controller = "Bank", Action = "IndexPartial"}
                };
            ...
            IEnumerable coll = ClientModel.GetBanks();
