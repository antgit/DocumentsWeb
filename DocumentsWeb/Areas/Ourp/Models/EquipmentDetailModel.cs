...
namespace DocumentsWeb.Areas.Ourp.Models
{
	public class EquipmentDetailModel
	{
		...
		/// <summary>Единица измерения (id)</summary>
		...
		/// <returns></returns>
        public EquipmentDetail ToObject()
		{
            EquipmentDetail obj = new EquipmentDetail();
			if (Id == 0)
                obj = new EquipmentDetail { Workarea = WADataProvider.WA };
			else
                obj = WADataProvider.WA.Cashe.GetCasheData<EquipmentDetail>().Item(Id);
		...
		public static List<EquipmentDetailModel> GetCollection(int id, bool refresh = false)
		{
			List<EquipmentDetailModel> list = new List<EquipmentDetailModel>();
			return list;
		}

		public static EquipmentDetailModel ConvertToModel(EquipmentDetail value)
		{
			EquipmentDetailModel obj = new EquipmentDetailModel
			{
				Id = value.Id,
				KindId = value.KindId,
				Name = value.Name,
        ...
        public static EquipmentDetailModel GetObject(int id)
        {
            EquipmentDetail e = WADataProvider.WA.Cashe.GetCasheData<EquipmentDetail>().Item(id);
            return EquipmentDetailModel.ConvertToModel(e);
        }
	}
}