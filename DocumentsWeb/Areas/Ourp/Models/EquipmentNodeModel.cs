...
{
	public class EquipmentNodeModel
	{
		...
		//TODO: Надо добавить Производитель+Номер и Норма трудозатрат+Разрядность

		public EquipmentNodeModel()
        {
		...
		/// <returns></returns>
        public EquipmentNode ToObject()
		{
			EquipmentNode obj = new EquipmentNode();
			if (Id == 0)
				obj = new EquipmentNode { Workarea = WADataProvider.WA };
			else
				obj = WADataProvider.WA.Cashe.GetCasheData<EquipmentNode>().Item(Id);
		...
		public static List<EquipmentNodeModel> GetCollection(int id, bool refresh = false)
		{
			List<EquipmentNodeModel> list = new List<EquipmentNodeModel>();
			return list;
		}

		public static EquipmentNodeModel ConvertToModel(EquipmentNode value)
		{
			EquipmentNodeModel obj = new EquipmentNodeModel
			{
				Id = value.Id,
				KindId = value.KindId,
				Name = value.Name,
		...
		public static EquipmentNodeModel GetObject(int id)
        {
			EquipmentNode e = WADataProvider.WA.Cashe.GetCasheData<EquipmentNode>().Item(id);
			return EquipmentNodeModel.ConvertToModel(e);
        }
	}
}