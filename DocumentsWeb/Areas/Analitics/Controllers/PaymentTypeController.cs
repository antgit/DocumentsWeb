...
namespace DocumentsWeb.Areas.Analitics.Controllers
{
    ...
    public class PaymentTypeController : AnaliticBaseController
    {
        /// <summary>
        /// Тип аналитики
        /// </summary>
        private const int AnaliticKindId = Analitic.KINDID_PAYMENTMETHOD;

        public PaymentTypeController()
        {
            Name = WebModuleNames.WEB_PAYMENTTYPE;
            RootHierachy = Hierarchy.SYSTEM_ANALITIC_TAXDOCPAYMENTMETHOD;
        }
        ...
        public ActionResult Edit([ModelBinder(typeof(DevExpressEditorsBinder))] AnaliticModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                HierarchyModel.UpdateHierarchiesContent<Analitic>(obj, model.InHierarchies, RootHierachy);

                /*if (model.Id == 0)
                {
                    Hierarchy h = DocumentsWeb.Models.WADataProvider.WA.Cashe.GetCasheData<Hierarchy>().ItemCode<Hierarchy>(RootHierachy);
        ...
        {
            AnaliticModel.ToTrash(id, RootHierachy);
        }
        ...
        public void ChangeState(int id, int state)
        {
            switch (state)
            {
                case State.STATEACTIVE:
                    AnaliticModel.SetStatetDone(id, RootHierachy);
                    break;
                case State.STATENOTDONE:
                    AnaliticModel.SetStateNotDone(id, RootHierachy);
                    break;
                case State.STATEDENY:
                    AnaliticModel.SetStateDeny(id, RootHierachy);
                    break;
            }
        }
        ...
        {
            AnaliticModel.CreateCopy(id, RootHierachy);
        }
        ...
        public ActionResult ExportTo(string type, string subtype)
        {
            ...
            List<AnaliticModel> coll = AnaliticModel.GetCollection(RootHierachy);
