...
{
    /// <summary>
    /// Контроллер объектов "Услуги"
    /// </summary>
    [MultiAuthorize(Roles = new[] { Uid.GROUP_GROUPWEBUSER })]
    public class ServiceController : ProductBaseController
    {
        public ServiceController()
        {
            Name = WebModuleNames.WEB_SERVICES;
            RootHierachy = Hierarchy.SYSTEM_PRODUCT_SERVICE;
        }
        ...
        {
            ProductModel model = new ProductModel { Id = 0, Name = string.Empty, KindId = Product.KINDID_SERVICE };
            WADataProvider.ModelsCache.Add(model.ModelId, model);
        ...
        public ActionResult Edit([ModelBinder(typeof(DevExpressEditorsBinder))] ProductModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                product.KindId = Product.KINDID_SERVICE;
        ...
        public void ChangeState(int id, int state)
        {
            switch (state)
            {
                case State.STATEACTIVE:
                    ProductModel.SetStatetDone(id, Hierarchy.SYSTEM_PRODUCT_SERVICE);
                    break;
                case State.STATENOTDONE:
                    ProductModel.SetStateNotDone(id, Hierarchy.SYSTEM_PRODUCT_SERVICE);
                    break;
                case State.STATEDENY:
                    ProductModel.SetStateDeny(id, Hierarchy.SYSTEM_PRODUCT_SERVICE);
                    break;
            }
        }
        ...
        {
            ProductModel.CreateCopy(id, Hierarchy.SYSTEM_PRODUCT_SERVICE);
        }
        ...
        public ActionResult ExportTo(string type)
        {
            GridViewSettings settings = new GridViewSettings
                {Name = "Товары", CallbackRouteValues = new {Action = "Index", Controller = "Service"}};
            ...
            IEnumerable coll = ProductModel.GetCollection(Hierarchy.SYSTEM_PRODUCT_SERVICE);
