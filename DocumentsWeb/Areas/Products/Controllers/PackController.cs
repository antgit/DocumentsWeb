...
{
    /// <summary>
    /// Контроллер объектов "Тара"
    /// </summary>
    [MultiAuthorize(Roles = new[] { Uid.GROUP_GROUPWEBUSER})]
    public class PackController : CoreController<Product>
    {
        public PackController()
        {
            Name = WebModuleNames.WEB_PRODUCTPACK;
            RootHierachy = Hierarchy.SYSTEM_PRODUCT_PACK;
        }
        ...
        {
            ProductModel model = new ProductModel { Id = 0, Name = string.Empty, KindId = Product.KINDID_PACK };
            WADataProvider.ModelsCache.Add(model.ModelId, model);
        ...
        public ActionResult Edit([ModelBinder(typeof(DevExpressEditorsBinder))] ProductModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                product.KindId = Product.KINDID_PACK;
                product.Save();
                if (model.Id == 0)
                {
                    //New
        ...
        public ActionResult ExportTo(string type)
        {
            GridViewSettings settings = new GridViewSettings
                {Name = "Товары", CallbackRouteValues = new {Action = "Index", Controller = "Pack"}};
            ...
            IEnumerable coll = ProductModel.GetCollection(Hierarchy.SYSTEM_PRODUCT_PACK);
