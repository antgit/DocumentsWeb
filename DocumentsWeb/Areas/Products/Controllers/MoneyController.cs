...
{
    /// <summary>
    /// Контроллер объектов "Денежные средства"
    /// </summary>
    [MultiAuthorize(Roles = new[] { Uid.GROUP_GROUPWEBUSER})]
    public class MoneyController : CoreController<Product>
    {
        public MoneyController()
        {
            Name = WebModuleNames.WEB_MONEY;
            RootHierachy = Hierarchy.SYSTEM_PRODUCT_MONEY;
        }
        ...
        {
            ProductModel model = new ProductModel { Id = 0, Name = string.Empty, KindId = Product.KINDID_MONEY };
            WADataProvider.ModelsCache.Add(model.ModelId, model);
        ...
        public ActionResult Edit([ModelBinder(typeof(DevExpressEditorsBinder))] ProductModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                product.KindId = Product.KINDID_MONEY;
                product.Save();
                if (model.Id == 0)
                {
                    //New
        ...
        public ActionResult Delete(int id)
        {
            ...
            p.Load(id);
            p.Delete();
            return RedirectToAction("Index", "Money");
        }
        ...
        public ActionResult ExportTo(string type)
        {
            GridViewSettings settings = new GridViewSettings
                {Name = "Товары", CallbackRouteValues = new {Action = "Index", Controller = "Money"}};
            ...
            IEnumerable coll = ProductModel.GetCollection(Hierarchy.SYSTEM_PRODUCT_MONEY);
