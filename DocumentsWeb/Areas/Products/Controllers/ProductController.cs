...
namespace DocumentsWeb.Areas.Products.Controllers
{
    ...
    public class ProductController : ProductBaseController
    {
        ...
        {
            Name = WebModuleNames.WEB_PRODUCTS;
            RootHierachy = Hierarchy.SYSTEM_PRODUCTS;
        }
        ...
        public ActionResult ControlView([ModelBinder(typeof(DevExpressEditorsBinder))] ProductModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    //New
                    Hierarchy hierarchy = WADataProvider.WA.Cashe.GetCasheData<Hierarchy>().ItemCode<Hierarchy>(Hierarchy.SYSTEM_PRODUCT_CURRENT);
        ...
        {
            ProductModel model = new ProductModel { Id = 0, Name = string.Empty, KindId = Product.KINDID_PRODUCT };
            WADataProvider.ModelsCache.Add(model.ModelId, model);
        ...
        public ActionResult ExportTo(string type)
        {
            GridViewSettings settings = new GridViewSettings
                {Name = "Товары", CallbackRouteValues = new {Action = "Index", Controller = "Product"}};
        ...
        {
            ProductModel model = ProductModel.GetObject(id);
            return Json(new { name = model.Name, nom = model.Nomenclature });
        }

        public ActionResult GetNameById(int id)
        {
            Product product = WADataProvider.WA.GetObject<Product>(id);
            return Content(product == null ? string.Empty : product.Name);
        }
    }
}
