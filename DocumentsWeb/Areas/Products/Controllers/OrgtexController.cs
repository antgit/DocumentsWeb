...
{
    /// <summary>
    /// Контроллер объектов "Орг техника"
    /// </summary>
    [MultiAuthorize(Roles = new[] { Uid.GROUP_GROUPWEBUSER})]
    public class OrgtexController : CoreController<Product>
    {
        public OrgtexController()
        {
            Name = WebModuleNames.WEB_ORGTEX;
            RootHierachy = Hierarchy.SYSTEM_PRODUCT_ORGTEX;
        }
        ...
        {
            ProductModel model = new ProductModel { Id = 0, Name = string.Empty, KindId = Product.KINDID_ORGTEX };
            WADataProvider.ModelsCache.Add(model.ModelId, model);
        ...
        public ActionResult Edit([ModelBinder(typeof(DevExpressEditorsBinder))] ProductModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                product.KindId = Product.KINDID_ORGTEX;
