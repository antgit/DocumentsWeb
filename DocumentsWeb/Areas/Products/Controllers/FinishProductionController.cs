...
{
    /// <summary>
    /// Контроллер объектов "Готовая продукция"
    /// </summary>
    [MultiAuthorize(Roles = new[] { Uid.GROUP_GROUPWEBUSER})]
    public class FinishProductionController : CoreController<Product>
    {
        public FinishProductionController()
        {
            Name = WebModuleNames.WEB_PRODUCTFINISHED;
            RootHierachy = Hierarchy.SYSTEM_PRODUCT_FINISHEDPRODUCTION;
        }
        ...
        {
            ProductModel model = new ProductModel { Id = 0, Name=string.Empty, KindId = Product.KINDID_FINISHPRODUCTION};
            WADataProvider.ModelsCache.Add(model.ModelId, model);
        ...
        public ActionResult Delete(int id)
        {
            ...
            p.Load(id);
            p.Delete();
            return RedirectToAction("Index", "FinishProduction");
        }
        ...
        public ActionResult ExportTo(string type)
        {
            GridViewSettings settings = new GridViewSettings
                {Name = "Товары", CallbackRouteValues = new {Action = "Index", Controller = "FinishProduction"}};
            ...
            IEnumerable coll = ProductModel.GetCollection(Hierarchy.SYSTEM_PRODUCT_FINISHEDPRODUCTION);
